...
# Pick whichever LLM client you're using
...
def _render_query_result(result: QueryResult):
    ...
    if not result.success:
        ...
        badge = "🗄️ SQL" if result.intent == "sql" else "🐍 Python"
        st.markdown(
        ...
        if result.sql:
            with st.expander("🗄️ SQL / Code", expanded=False):
...
if st.session_state.cleaned_df is None:
    ...
    if load_btn:
        ...
        try:
            ...
            llm         = get_llm("gemini", "gemini-2.5-flash")          # reads model config from .env / factory
...
if st.session_state.orchestrator is None:
    st.info("Load data into DuckDB (step ③) to enable querying.")
else:
    ...
    with col_opts:
        ...
        force_path = st.selectbox("Path", ["auto", "sql", "python"], index=0)
